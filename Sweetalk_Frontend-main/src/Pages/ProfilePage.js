              ...
              {profile.posts && profile.posts.length > 0 ? (
                <div className="grid grid-cols-3 gap-[1px] sm:gap-2">
                  {profile.posts.map((post) => (
                    <div key={post.postId} onClick={() => handleClick(post.postId)} className="aspect-[4/5] cursor-pointer overflow-hidden rounded-md">
                      <img src={post.imageUrl} className="w-full h-full object-cover transition hover:scale-105" />
                    </div>
                  ))}
                </div>
              ) : (
